...
export function classifyGsmSignal(value){
  const signal=Number(value);
  if(!Number.isFinite(signal)||signal<0||signal===99)return{value:null,percent:null,label:'Onbekend',level:'neutral'};
  const bounded=Math.min(31,signal),percent=Math.round(bounded/31*100);
  if(bounded<=4)return{value:bounded,percent,label:'Zeer zwak',level:'critical'};
  if(bounded<=9)return{value:bounded,percent,label:'Zwak',level:'warning'};
  if(bounded<=14)return{value:bounded,percent,label:'Redelijk',level:'warning'};
  if(bounded<=19)return{value:bounded,percent,label:'Goed',level:'ok'};
  return{value:bounded,percent,label:'Sterk',level:'ok'};
}
...
export function analyzeControllerLog(text){
  ...
  const ledMatches=[...text.matchAll(/LEDSTATE\s+CH\[(\d+)\]\s*state\[([^\]]+)\]/gi)],ppMatches=[...text.matchAll(/\bPP\[(\d+)\]\s*state\[([^\]]+)\]\[([^\]]+)\]/gi)],cpMatches=[...text.matchAll(/\bCP\[(\d+)\]\s*state\[([^\]]+)\](?:\[([^\]]+)\])?/gi)],readerSettings=[...text.matchAll(/"key"\s*:\s*"chg_Reader([12])"[^\r\n]*?"value"\s*:\s*"([^"]*)"/gi)].map(match=>({channel:Number(match[1]),value:match[2]})),socketType=values(/"key"\s*:\s*"chg_SktType"[^\r\n]*?"value"\s*:\s*"([^"]*)"/gi).at(-1)||null,webSocketPingInterval=Number(values(/(?:WS PING:\[|"key"\s*:\s*"WebSocketPingInterval"[^\r\n]*?"value"\s*:\s*")(\d+)/gi).at(-1)),lastWsTimeoutIndex=[...text.matchAll(/WS (?:PONG TIMEOUT|ERROR RX FRAME TO)/gi)].at(-1)?.index??-1,lastOcppTrafficIndex=[...text.matchAll(/OCPP (?:REQ|RESP|OUT|INPUT|Call result)/gi)].at(-1)?.index??-1;
  const stats={lines:text.split(/\r?\n/).filter(Boolean).length,boots:count(/State changed[^\n]*Boot|LEDSTATE[^\n]*Boot/gi),gsmPowerCycles:Math.floor(count(/GSM PWR (?:ON|OFF)/gi)/2),gsmResetNoNetwork:count(/GSM RST CAUSE:[^\n]*GSMNOK/gi),dhcpBound:count(/DHCP:\s*State BOUND/gi),dhcpStarts:count(/DHCP INIT/gi),dnsQueries:count(/DNS SERV|Connecting to DNS Server|Sending q/gi),dnsResolved:dnsResolved.length,httpConnects:count(/HTTP connect to/gi),webSocketErrors:count(/WS CONNECTION ERROR/gi),webSocketRxFrameTimeouts:count(/WS ERROR RX FRAME TO/gi),webSocketPongTimeouts:count(/WS PONG TIMEOUT/gi),webSocketTraffic:count(/OCPP (?:REQ|RESP|OUT|INPUT|Call result)/gi),ocppRecoveredAfterTimeout:lastWsTimeoutIndex>=0&&lastOcppTrafficIndex>lastWsTimeoutIndex,httpClientCloses:count(/HTTP CLIENT CLOSE/gi),meterTimeouts:count(/KWH:[^\n]*ERR\[TO\]/gi),readerErrors:count(/Reader init error/gi),readerReady:count(/Reader[^\r\n]*(?:initialized|active|ready|init OK)/gi),rfidReads:count(/(?:RFID|CARD|TAG)[^\r\n]*(?:UID|READ|DETECT|PRESENT)/gi),ledTransitions:ledMatches.length,ppSamples:ppMatches.length,cpSamples:cpMatches.length,modbusActive:count(/MODBUS Thread active/gi),maxTemperatureC:temperatures.length?Math.max(...temperatures):null,minSupplyMv:supplies.length?Math.min(...supplies):null};
  const webSocketStatus=stats.ocppRecoveredAfterTimeout?'Time-out gezien · OCPP bleef actief':stats.webSocketPongTimeouts?'Pong-time-out':stats.webSocketErrors?'Verbindingsfout':stats.webSocketRxFrameTimeouts&&stats.webSocketTraffic?'Vertraagde frames · OCPP bleef actief':stats.webSocketRxFrameTimeouts?'RX-frame-time-out':stats.webSocketTraffic?'OCPP-verkeer bevestigd':Number.isFinite(webSocketPingInterval)?`Keepalive ${webSocketPingInterval} sec ingesteld`:'Niet in deze log bevestigd',rfidConfigured=readerSettings.some(row=>!/^none(?:,|$)/i.test(row.value)),rfidStatus=stats.readerErrors?'Lezerstoring':stats.readerReady?'Lezer actief':rfidConfigured?'Geconfigureerd · geen kaartlezing gezien':readerSettings.length?'Niet geconfigureerd':'Niet in deze log gemeten',lastLed=ledMatches.at(-1),lastPp=ppMatches.at(-1),lastCp=cpMatches.at(-1),gsmQuality=classifyGsmSignal(gsmSignal);
  const facts={localIp,gateway,dnsServer,dnsResolved,httpTargets,gsmRegistration,gsmSignal,gsmQuality,webSocketStatus,webSocketPingInterval:Number.isFinite(webSocketPingInterval)?webSocketPingInterval:null,rfidStatus,rfidConfigured,readerSettings,ledState:lastLed?.[2]||null,ledChannel:lastLed?Number(lastLed[1])+1:null,ppState:lastPp?.[2]||null,ppValue:lastPp?.[3]||null,ppChannel:lastPp?Number(lastPp[1])+1:null,cpState:lastCp?.[2]||null,cpValue:lastCp?.[3]||null,cpChannel:lastCp?Number(lastCp[1])+1:null,socketType,retryDelayMinutes:retryDelays.at(-1)??null,modem:values(/GSM Modem:\s*([^\r\n]+)/gi).at(-1)||null,operator:values(/GSM preferred operator \[([^\]]+)\]/gi).at(-1)||null};
  const eventPatterns=[['boot',/Application start|LEDSTATE[^\n]*Boot/i],['gsm',/GSM (?:RST CAUSE|Modem:|REG:|preferred operator)|GSM CCID|GSM IMSI/i],['ethernet',/ETH0 |DHCP: State|IP address|gateway|DNS server/i],['dns',/DNS (?:SERV|RESOLVED)/i],['tcp',/HTTP connect to/i],['websocket',/WS (?:CONNECTION ERROR|PONG TIMEOUT|ERROR RX FRAME TO)|HTTP CLIENT CLOSE|OCPP (?:REQ|RESP)/i],['retry',/EV RETRY/i],['meter',/KWH:|Meter\d*:SN/i],['rfid',/Reader|RFID|CARD[^ ]|TAG\[/i],['pilot',/\b(?:CP|PP)\[/i],['led',/LEDSTATE/i]];
  ...
  if(stats.webSocketPongTimeouts)findings.push({level:stats.ocppRecoveredAfterTimeout?'warning':'critical',code:'WS_PONG_TIMEOUT',title:stats.ocppRecoveredAfterTimeout?'WebSocket herstelde na PONG-time-out':'WebSocket-keepalive krijgt geen antwoord',evidence:`${stats.webSocketPongTimeouts} PONG-time-out${stats.webSocketPongTimeouts===1?'':'s'}${stats.ocppRecoveredAfterTimeout?' · daarna opnieuw OCPP-verkeer':''}`,action:stats.ocppRecoveredAfterTimeout?'De sessie viel niet blijvend weg. Vergelijk het tijdstip met de FTP-upload en bewaak of de time-outs toenemen.':'DNS en TCP kunnen hierbij gewoon werken. Controleer de WebSocket-proxy, OCPP-subprotocol en WebSocketPingInterval; een modemreset is pas zinvol als ook de netwerkregistratie wegvalt.'});
  if(stats.webSocketRxFrameTimeouts)findings.push({level:'warning',code:'WS_RX_FRAME_DELAY',title:'WebSocket-frames komen vertraagd binnen',evidence:`${stats.webSocketRxFrameTimeouts} RX-frame-time-out${stats.webSocketRxFrameTimeouts===1?'':'s'}${stats.ocppRecoveredAfterTimeout?' · OCPP-verkeer ging daarna door':''}`,action:'Controleer overlap met de FTP-diagnose-upload. Beperk de uploadgrootte en geef OCPP-verkeer voorrang op dezelfde mobiele verbinding.'});
...
export function extractDiagnosticOverview(value){
  ...
  const pgrid=[...text.matchAll(/PGrid\[([^\]]+)\]MIN\.I\[([^\]]+)\]STATION\[([^\]]+)\]INSTALLATION\[([^\]]+)\]SUPERVISOR\[([^\]]+)\]/gi)].at(-1),supervisorClientCount=number(configuration.grid_SupervisorClientCount),runtimeSupervisor=number(pgrid?.[5]),runtimeSlaveCount=number([...text.matchAll(/SLAVECOUNT\[(\d+)\]/gi)].at(-1)?.[1]);
...
export function extractCellularIdentity(text){
  ...
  return {
    ...
    signal,
    signalQuality:classifyGsmSignal(signal),
    registrationCode,
    registration,
