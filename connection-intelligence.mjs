...
export function extractDiagnosticOverview(value){
  ...
  const canRxFrames=(text.match(/CAN(?:BUS)?\s*(?:RX|RECV|RECEIVE)(?!\s+RINGBUFFER)[^\r\n]*(?:\bID\b|ID\[|0x[0-9a-f]{2,}|DATA\[)/gi)||[]).length,canTxFrames=(text.match(/CAN(?:BUS)?\s*(?:TX|SEND|TRANSMIT)(?!\s+RINGBUFFER)[^\r\n]*(?:\bID\b|ID\[|0x[0-9a-f]{2,}|DATA\[)/gi)||[]).length,gridRole=configuration.grid_Role||null,canConfigured=String(configuration.grid_CommChannel||'').toLowerCase()==='canbus',canPeerExpected=/^(?:master|slave)$/i.test(String(gridRole||'')),canPeerDetected=canRxFrames>0||(runtimeSupervisor||0)>0;
