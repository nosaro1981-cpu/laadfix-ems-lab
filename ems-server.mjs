...
import { connectionIntelligence, analyzeControllerLog, assessMeterIdentity, extractCellularIdentity, extractMeterIdentity, extractDiagnosticOverview, normalizeControllerLog, readableControllerLog, diagnosticAnalysisWindow } from './connection-intelligence.mjs';
...
export async function startEMS({port=8080,host='127.0.0.1',hardware=true,ledHardware=hardware,publicHost=null,authUser=null,authPassword=null,relayMonitorPort=8081,fleetProvider=null,fleetRouteChanger=null,fleetCommander=null,meterPollIntervalMs=30000,diagnosticCaptureMs=null}={}) {
  ...
  const diagnosticTokens=new Map(),diagnosticReports=new Map(),diagnosticHistories=new Map(),diagnosticHistoryLoads=new Map(),importedRemoteDiagnostics=new Set();
  ...
  const server = http.createServer(async(req,res)=>{
    ...
    if(req.method==='POST'&&requestPath==='/api/diagnostics-manual'){
      ...
      try{const chunks=[];let bytes=0;for await(const chunk of req){bytes+=chunk.length;if(bytes>5*1024*1024)throw Error('Diagnosebestand is groter dan 5 MB');chunks.push(chunk);}const ticket={chargerId,requestedAt:new Date().toISOString(),source:'Handmatige browserupload',destination:'LaadFix online analyse',locationHost:publicName,expiresAt:Date.now()+15*60_000,fileName};const report=buildDiagnosticReport(chargerId,ticket,Buffer.concat(chunks));await rememberDiagnosticReport(chargerId,report);return send(201,{report});}catch(e){return send(400,{error:e.message});}
    }
      ...
      if(req.url==='/api/fleet-command'){
        ...
        if(action==='diagnostics'&&diagnosticTokens.get(diagnosticToken)?.localReceiver&&body.enhancedDebug!==false){
          ...
          if(body.freshStart===true){
            ...
            diagnosticReports.set(chargerId,{...ticket,status:'Controller herstarten voor frisse diagnose',transport:'Lokale FTP + HTTPS'});
          ...
          diagnosticReports.set(chargerId,{...ticket,status:'Uitgebreide logging verzamelen',transport:'Lokale FTP + HTTPS',debugRestoreStatus:'Wordt na ontvangst automatisch hersteld'});
          ...
          setTimeout(async()=>{try{const requestedAt=ocppDateTime(Date.now()),startTime=ocppDateTime(Date.now()-captureMs);ticket.requestedAt=requestedAt;ticket.startTime=startTime;ticket.stopTime=requestedAt;const result=await fleetCommander(chargerId,'GetDiagnostics',{location:diagnosticLocation,retries:2,retryInterval:60,startTime,stopTime:requestedAt});ticket.fileName=result?.fileName||null;diagnosticReports.set(chargerId,{...diagnosticReports.get(chargerId),...ticket,status:ticket.fileName?'Lokale upload wordt gevolgd':'Mislukt',fileName:ticket.fileName,controllerResponse:result});if(!ticket.fileName)await restoreDiagnosticDebug(ticket);}catch(error){await restoreDiagnosticDebug(ticket);diagnosticReports.set(chargerId,{...diagnosticReports.get(chargerId),status:'Mislukt',error:error.message,debugRestoreStatus:ticket.debugRestoreStatus});}},captureMs).unref();
