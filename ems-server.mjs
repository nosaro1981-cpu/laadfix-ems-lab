...
import { connectionIntelligence, analyzeControllerLog, assessMeterIdentity, classifyGsmSignal, extractCellularIdentity, extractMeterIdentity, extractMeterIdentities, extractDiagnosticOverview, normalizeControllerLog, readableControllerLog, compactReadableControllerLog, diagnosticAnalysisWindow } from './connection-intelligence.mjs';
...
const DIAGNOSTIC_SETTING_KEYS=['chg_Debug','RestartTransOnBoot','ZeRdy','CircDbg','NoBeep','ChgProf','chkOnlineAuth','chkOnlineAuthStop','StopOnAnyCrd','AllowAllCards','comMaster','Events','BlockBeforeBoot','Wdt','updSendInIdle','UseTLS','blockLgFull','eth cfg'];
export function diagnosticConfigurationSnapshot(configuration=[]){
  const rows=new Map((Array.isArray(configuration)?configuration:[]).map(row=>[String(row.key||'').trim().toLowerCase(),row]));
  return DIAGNOSTIC_SETTING_KEYS.flatMap(key=>{const row=rows.get(key.toLowerCase());return row?[{key,value:String(row.value??''),readonly:!!row.readonly}]:[];});
}
...
export function diagnosticTextWithSettings(report={},value=''){
  const settings=Array.isArray(report.diagnosticSettings)?report.diagnosticSettings:[],debug=report.diagnosticDebugPlan||{},lines=['[LaadFix veilige configuratiemomentopname]',...settings.map(row=>`${row.key}=${row.value}`)];
  if(debug.original)lines.push(`chg_Debug vóór diagnose=${debug.original}`);
  if(debug.temporary)lines.push(`chg_Debug tijdelijk=${debug.temporary}`);
  lines.push('','[Leesbare controllerlog]',downloadableDiagnosticText(value));
  return lines.join('\n');
}
...
export async function startEMS({port=8080,host='127.0.0.1',hardware=true,ledHardware=hardware,publicHost=null,authUser=null,authPassword=null,relayMonitorPort=8081,fleetProvider=null,fleetRegistrar=null,fleetRouteChanger=null,fleetCommander=null,meterPollIntervalMs=30000,diagnosticCaptureMs=null,diagnosticLocalTimeoutMs=null,diagnosticReconnectWaitMs=90_000,diagnosticConfigurationTimeoutMs=10_000,diagnosticFtpUrlOverride=null,diagnosticFtpLister=null,diagnosticFtpSnapshotter=null}={}) {
  ...
  const persistDiagnosticText=async(chargerId,report,content)=>{if(!diagnosticFtpUrl)return null;const client=new FtpClient(15000),textFileName=diagnosticTextFileName(chargerId,report),text=Buffer.from(diagnosticTextWithSettings(report,content),'utf8');try{const{directory}=await diagnosticFtpAccess(client);await client.uploadFrom(Readable.from([text]),diagnosticRemotePath(directory,textFileName));return textFileName;}finally{client.close();}};
  ...
  const dashboardDiagnosticReport=(report,{history=false}={})=>{if(!report)return report;const{excerpt,liveRaw,livePreview,...summary}=report;if(history)return summary;const preview=livePreview?{...livePreview,excerpt:String(livePreview.excerpt||'').slice(-4000),liveRaw:String(livePreview.liveRaw||'').slice(-20000)}:undefined;return{...summary,excerpt:String(excerpt||'').slice(-4000),...(preview?{livePreview:preview}:{})};};
  const dashboardStation=(item,{details=false}={})=>{if(!item)return item;return{...item,events:details&&Array.isArray(item.events)?item.events.slice(0,12):[],ocppMessages:details&&Array.isArray(item.ocppMessages)?item.ocppMessages.slice(0,30):[],connectionTimeline:details&&Array.isArray(item.connectionTimeline)?item.connectionTimeline.slice(0,20):[],meterHistory:details&&Array.isArray(item.meterHistory)?item.meterHistory.slice(0,30):[],configuration:details&&Array.isArray(item.configuration)?item.configuration:[]};};
  ...
  const buildDiagnosticReport=(chargerId,ticket,content)=>{
    const source=Buffer.isBuffer(content)?content.toString('utf8'):String(content),item=(typeof fleetProvider==='function'?fleetProvider():[]).find(row=>row.id===chargerId),configuration=ticket.configuration||item?.configuration||[],configurationText=configuration.map(row=>JSON.stringify({key:row.key,readonly:!!row.readonly,value:String(row.value??'')})).join('\n'),text=normalizeControllerLog(source+'\n'+configurationText),analysis=analyzeControllerLog(diagnosticAnalysisWindow(text)),meterSettings=ticket.meterSettings||configuration.filter(row=>/^chg_KWH[12]$/i.test(row.key)),overview=extractDiagnosticOverview(text),configurationMap=Object.fromEntries(configuration.map(row=>[row.key,String(row.value??'')])),logCellular=extractCellularIdentity(text),signal=logCellular.signal||configurationMap.gsm_SigQ||null,cellular={...logCellular,modem:logCellular.modem||configurationMap.gsm_Model||null,operator:logCellular.operator||configurationMap.gsm_Oper||null,signal,signalQuality:classifyGsmSignal(signal)},ethRaw=configurationMap['eth cfg']||configurationMap.eth_cfg||'',ethConfiguration=Object.fromEntries(String(ethRaw).split(',').map(part=>part.split('=').map(value=>value.trim())).filter(part=>part.length===2)),networkConfiguration={mode:ethConfiguration.type||null,localIp:ethConfiguration.ip||null,netmask:ethConfiguration.netmask||null,dnsServer:ethConfiguration.dns||null,gateway:ethConfiguration.gw||null,source:ethRaw?'controllerconfiguratie':null},excerpt=readableControllerLog(source).slice(-20000).replace(/ftps?:\/\/[^\s"'<>]+/gi,'[FTP-afgeschermd]');
    ...
    const receivedAt=new Date().toISOString(),uploadStartedAt=ticket.uploadStartedAt||ticket.requestedAt||null,uploadDurationSeconds=uploadStartedAt?Math.max(0,Math.round((Date.parse(receivedAt)-Date.parse(uploadStartedAt))/1000)):null;
    return{chargerId,status:'Ontvangen',quickMode:ticket.quickMode===true,fastScan:ticket.fastScan===true,debugModules:Array.isArray(ticket.debugModules)?[...ticket.debugModules]:[],progress:{phase:'complete',label:ticket.quickMode?'Snelle diagnose gereed':'Diagnose gereed',percent:100},requestedAt:ticket.requestedAt,startTime:ticket.startTime||null,stopTime:ticket.stopTime||null,minutes:ticket.minutes??null,durationSeconds:ticket.durationSeconds??(ticket.minutes==null?null:ticket.minutes*60),source:ticket.source||'LaadFix',destination:ticket.destination||diagnosticDestination,locationHost:ticket.locationHost||diagnosticFtpHost||null,receivedAt,uploadStartedAt,uploadDurationSeconds,uploadCompleted:true,captureLimitBytes:ticket.captureLimitBytes||diagnosticSmartCaptureBytes,fileName:ticket.fileName||null,bytes:Buffer.byteLength(content),controllerStatus:item?.diagnosticsStatus||null,meterConfiguration,meterIdentity,meterAssessment,meterSlots,diagnosticSettings:diagnosticConfigurationSnapshot(configuration),diagnosticDebugPlan:{original:ticket.originalDebug||configurationMap.chg_Debug||null,temporary:ticket.maximumDebug||null},overview,cellular,networkConfiguration,analysis,excerpt};
  };
  ...
  const scheduleFtpDiagnosticDownload=(chargerId,ticket)=>{
    ...
    const run=async()=>{
      ...
      try{
        ...
        if(!ticket.uploadStartedAt)ticket.uploadStartedAt=new Date().toISOString();
        ...
        if(diagnosticLivePreviewShouldRead(entry.size,ticket.previewSourceSize)){
          try{
            ...
            if(snapshotContent.length){
              const preview={...buildDiagnosticReport(chargerId,ticket,snapshotContent),status:'Live uitlezing',smartCapture:true,liveRaw:readableControllerLog(snapshotContent).slice(-captureLimitBytes),truncated:entry.size>snapshotContent.length,sourceUploadBytes:entry.size,bytes:snapshotContent.length,receivedAt:new Date().toISOString()};
        ...
        if(ticket.fastScan&&(ticket.finishRequested||captureBoundaryReached)){
          ...
          const finalSnapshot=await readGrowingDiagnosticSnapshot(url,remote,entry.size,previewSourceLimit),content=compactReadableControllerLog(finalSnapshot,captureLimitBytes),preview={...buildDiagnosticReport(chargerId,ticket,content),status:'Live uitlezing',smartCapture:true,liveRaw:readableControllerLog(content).slice(-captureLimitBytes),truncated:entry.size>content.length,sourceUploadBytes:entry.size,bytes:content.length,receivedAt:new Date().toISOString()};
          ticket.previewSourceSize=entry.size;ticket.livePreview=preview;
          ...
          if(entry.size>=captureLimitBytes||ticket.finishRequested){
            updateDiagnosticProgress(chargerId,ticket,{phase:'restoring',label:'Oorspronkelijke debug herstellen',percent:94,estimatedCompleteAt:new Date(Date.now()+15_000).toISOString()},{status:'Debuginstelling herstellen'});
            const restored=await restoreDiagnosticDebug(ticket);
            const {liveRaw,...finalPreview}=preview;
            const report={...finalPreview,status:'Ontvangen',debugRestoreStatus:ticket.originalDebug?(ticket.debugRestored||restored?'Originele debuginstelling hersteld':ticket.debugRestoreStatus):null,progress:{phase:'complete',label:'Compacte diagnose gereed',percent:100}};
  ...
  const server = http.createServer(async(req,res)=>{
    ...
    if(req.method==='GET'&&requestPath==='/api/state'){const stationId=String(requestUrl.searchParams.get('station')||''),view=String(requestUrl.searchParams.get('view')||''),emsView=['overview','ems'].includes(view),dashboardCharger=dashboardStation(charger,{details:charger.id===stationId});dashboardCharger.fleet=Array.isArray(charger.fleet)?charger.fleet.map(item=>dashboardStation(item,{details:item.id===stationId})):[];const dashboardState={...state,result:emsView?state.result:null,history:emsView?state.history:[]},diagnostics=stationId?(diagnosticReports.has(stationId)?{[stationId]:dashboardDiagnosticReport(diagnosticReports.get(stationId))}:{}):Object.fromEntries([...diagnosticReports].map(([id,report])=>[id,dashboardDiagnosticReport(report)])),diagnosticHistory=stationId?(diagnosticHistories.has(stationId)?{[stationId]:(diagnosticHistories.get(stationId)||[]).slice(0,20).map(report=>dashboardDiagnosticReport(report,{history:true}))}:{}):Object.fromEntries([...diagnosticHistories].map(([id,rows])=>[id,rows.slice(0,20).map(report=>dashboardDiagnosticReport(report,{history:true}))]));const payload={...dashboardState,simulationFleet:emsView?simulatedFleet(state.settings,state.result):[],diagnostic,diagnosticTransport:{online:!!diagnosticFtpUrl,destination:diagnosticDestination},charger:dashboardCharger,recovery:{...recoveryCenter.snapshot(),...recoveryCases.snapshot(),stations:recoveryStations().map(item=>({id:item.id,chargerConnected:item.chargerConnected,backendConnected:item.backendConnected,status:item.status,connectors:item.connectors||{},connectorIds:connectorIds(item),blockedReason:recoveryGuard(item),lastSeen:item.lastSeen}))},meter:{...extractMeterReadings(charger.meterValues,charger.lastMeterValues),forwardedAt:charger.lastMeterForwarded||null,historyCount:charger.meterHistoryCount||0,lastRequest:lastMeterRequest?new Date(lastMeterRequest).toISOString():null,requestResult:meterRequestResult},led,service:assessService(charger),intelligence:connectionIntelligence(charger,connectionSamples),logAnalysis,diagnostics,diagnosticHistory,powerRecovery:powerRecovery||recoveryMonitor.snapshot(charger),serviceResult,networkResult,control:{liveControl,lastSentLimit,lastControlError,lastControlResult,capabilities}},plain=Buffer.from(JSON.stringify(payload)),stablePayload=emsView?payload:{...payload,service:null,intelligence:null,etagWindow:Math.floor(Date.now()/30_000)},etag='"'+createHash('sha256').update(JSON.stringify(stablePayload)).digest('base64url').slice(0,20)+'"';if(req.headers['if-none-match']===etag){res.writeHead(304,{'ETag':etag,'Cache-Control':'private, no-cache'});res.end();return;}const gzip=/\bgzip\b/i.test(String(req.headers['accept-encoding']||'')),body=gzip?gzipSync(plain,{level:6}):plain;res.writeHead(200,{'Content-Type':'application/json','Content-Length':body.length,'Cache-Control':'private, no-cache','ETag':etag,...(gzip?{'Content-Encoding':'gzip','Vary':'Accept-Encoding'}:{})});res.end(body);return;}
    ...
    try{
      ...
      if(req.url==='/api/fleet-command'){
        ...
        if(action==='diagnostics'&&body.enhancedDebug!==false){
          ...
          const configurationTimeout=Math.max(1000,Math.min(30_000,Number(diagnosticConfigurationTimeoutMs)||10_000)),liveCachedRows=Array.isArray(item.configuration)?item.configuration:[],knownRows=KNOWN_DIAGNOSTIC_CONFIGURATION[chargerId]||[],cachedRows=[...new Map([...knownRows,...liveCachedRows].map(row=>[String(row.key||'').toLowerCase(),row])).values()];
