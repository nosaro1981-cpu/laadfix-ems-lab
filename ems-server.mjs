    ...
    try{
      ...
      if(req.url==='/api/fleet-command'){
        ...
        if(action==='diagnostics'&&diagnosticTokens.get(diagnosticToken)?.localReceiver&&body.enhancedDebug!==false){
          ...
          const captureMs=diagnosticCaptureMs===null?Math.max(60_000,(ticket.minutes||5)*60_000):Math.max(1,Number(diagnosticCaptureMs));
          ...
          return send(202,{serviceResult:{status:'Uitgebreide logging gestart',steps:[`Originele chg_Debug veilig bewaard`,`Relevante modules tijdelijk op niveau 7`,`Diagnose wordt over ${ticket.minutes||5} minuten automatisch opgevraagd`],advice:'Na ontvangst of uiterlijk na twaalf minuten wordt de oorspronkelijke debuginstelling automatisch teruggezet.'}});
        }
        const commands={
