      ...
      if(req.url==='/api/fleet-command'){
        ...
        if(action==='diagnostics'&&diagnosticTokens.get(diagnosticToken)?.localReceiver&&body.enhancedDebug!==false){
          ...
          if(body.freshStart===true){
            ...
            updateDiagnosticProgress(chargerId,ticket,{phase:'restarting',label:'Controller opnieuw verbinden',percent:8,estimatedCompleteAt:new Date(Date.now()+120_000).toISOString()},{status:'Controller herstarten voor frisse diagnose',transport:'Lokale FTP + HTTPS'});
