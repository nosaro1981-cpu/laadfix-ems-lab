      ...
      if(req.url==='/api/fleet-command'){
        ...
        busy=true;try{if(action==='diagnostics'&&!item.configuration?.some(row=>row.key==='chg_KWH1'))await fleetCommander(chargerId,'GetConfiguration',{key:['chg_KWH1']});const [ocppAction,payload]=commands[action],beforeStatus=item.lastStatusNotification,beforeMeter=item.lastMeterValues,result=await fleetCommander(chargerId,ocppAction,payload);let update=null;
          ...
          if(action==='status'){
            ...
            if(update)steps.push(`Connector 1: ${update.status||'Onbekend'}`,`Foutcode: ${update.errorCode||'NoError'}`,`Ontvangen: ${new Date(update.lastStatusNotification).toLocaleString('nl-NL')}`);
          ...
          if(action==='meterValues'){
            ...
            if(update){const row=update.meterHistory?.[0],value=(sample,fallback='niet meegestuurd')=>sample&&Number.isFinite(Number(sample.value))?`${Number(sample.value).toLocaleString('nl-NL',{maximumFractionDigits:3})} ${sample.unit||''}`.trim():fallback;steps.push(`Meting: ${new Date(update.lastMeterValues).toLocaleString('nl-NL')}`,`Energiestand: ${value(row?.energy)}`,`Spanning L1: ${value(row?.voltageL1)}`,`Stroom L1: ${value(row?.currentL1)}`,`Frequentie: ${value(row?.frequency)}`,`Temperatuur: ${value(row?.temperature)}`,update.lastMeterForwarded?'Doorgestuurd naar Robo Charge':'Nog niet doorgestuurd naar Robo Charge');}
