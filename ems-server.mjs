...
export function diagnosticStationResponsive(item,now=Date.now(),maxAgeMs=180_000){
  const connection=item?.connectionDiagnostics,raw=connection?connection.lastChargerMessageAt:item?.gatewayHealth?.lastMessageAt,last=typeof raw==='number'?raw:raw?Date.parse(raw):NaN,readinessKnown=!!connection&&('chargerTrafficSeen'in connection||'chargerTransportResponsive'in connection||'lastChargerMessageAt'in connection);
  return !!item?.chargerConnected&&(!readinessKnown||!item?.commandHealth?.degraded&&(item?.commandHealth?.status==='healthy'||Number.isFinite(last)&&now-last<=maxAgeMs));
}
