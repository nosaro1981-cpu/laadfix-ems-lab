...
export async function startEMS({port=8080,host='127.0.0.1',hardware=true,ledHardware=hardware,publicHost=null,authUser=null,authPassword=null,relayMonitorPort=8081,fleetProvider=null,fleetRouteChanger=null,fleetCommander=null,meterPollIntervalMs=30000,diagnosticCaptureMs=null}={}) {
  ...
  const diagnosticFtpMaxAttempts=Math.max(2,Math.ceil((5*60_000)/diagnosticFtpPollMs));
  ...
  const requestShortDiagnostics = async (chargerId, minutes = 5) => {
    ...
    const ticket = { chargerId, requestedAt, startTime, stopTime, minutes, source:'LaadFix', destination:diagnosticDestination, locationHost:diagnosticFtpHost||publicName, expiresAt: Date.now() + 15 * 60000, fileName: null };
  ...
  const buildDiagnosticReport=(chargerId,ticket,content)=>{
    ...
    return{chargerId,status:'Ontvangen',progress:{phase:'complete',label:'Diagnose gereed',percent:100},requestedAt:ticket.requestedAt,startTime:ticket.startTime||null,stopTime:ticket.stopTime||null,minutes:ticket.minutes??null,durationSeconds:ticket.durationSeconds??(ticket.minutes==null?null:ticket.minutes*60),source:ticket.source||'LaadFix',destination:ticket.destination||diagnosticDestination,locationHost:ticket.locationHost||diagnosticFtpHost||null,receivedAt:new Date().toISOString(),fileName:ticket.fileName||null,bytes:Buffer.byteLength(content),controllerStatus:item?.diagnosticsStatus||null,meterConfiguration,meterIdentity,meterAssessment,meterSlots,overview,cellular,analysis,excerpt};
  };
  const scheduleFtpDiagnosticDownload=(chargerId,ticket)=>{let attempts=0,lastSize=null,stable=0;updateDiagnosticProgress(chargerId,ticket,{phase:'uploading',label:'FTP-upload volgen',percent:65,estimatedCompleteAt:new Date(Date.now()+120_000).toISOString()});const run=async()=>{attempts++;const client=new FtpClient(15000);try{const url=new URL(diagnosticFtpUrl),fileName=String(ticket.fileName||'').split(/[\\/]/).at(-1);if(!fileName)throw Error('Homebox heeft geen bestandsnaam gemeld');await client.access({host:url.hostname,port:Number(url.port||21),user:decodeURIComponent(url.username),password:decodeURIComponent(url.password),secure:url.protocol==='ftps:'});const directory=decodeURIComponent(url.pathname||'/').replace(/\/$/,'')||'/',entries=await client.list(directory),entry=entries.find(row=>row.name===fileName);if(!entry)throw Error('Bestand staat nog niet op de FTP-server');stable=entry.size===lastSize?stable+1:0;lastSize=entry.size;if(stable<1)throw Error('Upload is nog bezig');updateDiagnosticProgress(chargerId,ticket,{phase:'analyzing',label:'Bestand analyseren',percent:88,estimatedCompleteAt:new Date(Date.now()+15_000).toISOString()},{status:'Log analyseren'});const chunks=[];let bytes=0;const sink=new Writable({write(chunk,encoding,callback){bytes+=chunk.length;if(bytes>5*1024*1024)return callback(Error('Diagnosebestand is groter dan 5 MB'));chunks.push(Buffer.from(chunk));callback();}});const remote=(directory==='/'?'':directory)+'/'+fileName;await client.downloadTo(sink,remote);const report=buildDiagnosticReport(chargerId,ticket,Buffer.concat(chunks));updateDiagnosticProgress(chargerId,ticket,{phase:'restoring',label:'Oorspronkelijke debug herstellen',percent:94,estimatedCompleteAt:new Date(Date.now()+10_000).toISOString()},{status:'Debuginstelling herstellen'});const restored=await restoreDiagnosticDebug(ticket);await rememberDiagnosticReport(chargerId,{...report,enhancedDebug:!!ticket.enhancedDebug,debugRestoreStatus:ticket.originalDebug?(restored?'Originele debuginstelling hersteld':ticket.debugRestoreStatus):null});return;}catch(error){const failed=attempts>=diagnosticFtpMaxAttempts;if(failed&&ticket.originalDebug){updateDiagnosticProgress(chargerId,ticket,{phase:'restoring',label:'Debug veilig herstellen',percent:94,estimatedCompleteAt:new Date(Date.now()+10_000).toISOString()},{status:'Debuginstelling herstellen na overdrachtsfout'});await restoreDiagnosticDebug(ticket);}updateDiagnosticProgress(chargerId,ticket,{phase:failed?'failed':'uploading',label:failed?'FTP-upload mislukt':'Wachten op FTP-bestand',percent:failed?100:Math.min(84,65+attempts),estimatedCompleteAt:new Date(Date.now()+Math.max(15_000,(diagnosticFtpMaxAttempts-attempts)*diagnosticFtpPollMs)).toISOString()},{status:failed?'Mislukt':'FTP-upload wordt gevolgd',error:String(error.message||'FTP-fout').replace(diagnosticFtpUrl,'FTP-server'),ftpAttempt:attempts,debugRestoreStatus:ticket.debugRestoreStatus});if(!failed)setTimeout(run,diagnosticFtpPollMs).unref();}finally{client.close();}};setTimeout(run,Math.min(10000,diagnosticFtpPollMs)).unref();};
      ...
      if(req.url==='/api/fleet-command'){
        ...
        if(action==='diagnostics'&&body.enhancedDebug!==false){
          ...
          if(body.freshStart===true){
            ...
            if(reset?.status!=='Accepted')throw Error('Soft reset voor frisse diagnose is niet geaccepteerd');
            ...
            active=!!item.activeTransaction||['Charging','Preparing','Finishing'].includes(item.status);
          }
          updateDiagnosticProgress(chargerId,ticket,{phase:'configuration',label:'Actuele configuratie lezen',percent:14,estimatedCompleteAt:new Date(Date.now()+(ticket.durationSeconds||300)*1000+130_000).toISOString()},{status:'Actuele configuratie lezen'});
          ...
          ticket.meterSettings=rows.filter(row=>/^chg_KWH[12]$/i.test(row.key)).map(row=>({key:row.key,value:row.value}));ticket.freshStart=body.freshStart===true;
          updateDiagnosticProgress(chargerId,ticket,{phase:'debugging',label:'Debugmodules maximaal zetten',percent:18,estimatedCompleteAt:new Date(Date.now()+(ticket.durationSeconds||300)*1000+120_000).toISOString()},{status:'Debugniveau tijdelijk verhogen'});
          ...
          ticket.originalDebug=originalDebug;ticket.maximumDebug=maximumDebug;ticket.enhancedDebug=true;ticket.captureStartedAt=new Date().toISOString();
          ...
          const captureStartedAt=Date.now();updateDiagnosticProgress(chargerId,ticket,{phase:'capturing',label:'Maximale logging verzamelen',percent:20,phaseStartedAt:new Date(captureStartedAt).toISOString(),phaseEndsAt:new Date(captureStartedAt+captureMs).toISOString(),estimatedCompleteAt:new Date(captureStartedAt+captureMs+(ticket.transferEstimateMs||100_000)+20_000).toISOString()},{status:'Uitgebreide logging verzamelen',transport:ticket.localReceiver?'Lokale FTP + HTTPS':'LaadFix FTP',debugRestoreStatus:'Wordt na ontvangst automatisch hersteld'});
          setTimeout(async()=>{try{updateDiagnosticProgress(chargerId,ticket,{phase:'requesting',label:'Diagnosebestand opvragen',percent:55,estimatedCompleteAt:new Date(Date.now()+(ticket.transferEstimateMs||100_000)+15_000).toISOString()},{status:'Diagnosebestand opvragen'});const requestedAt=ocppDateTime(Date.now()),startTime=ocppDateTime(Date.now()-captureMs);ticket.requestedAt=requestedAt;ticket.startTime=startTime;ticket.stopTime=requestedAt;const result=await fleetCommander(chargerId,'GetDiagnostics',{location:diagnosticLocation,retries:2,retryInterval:60,startTime,stopTime:requestedAt});ticket.fileName=result?.fileName||null;updateDiagnosticProgress(chargerId,ticket,{phase:ticket.fileName?'uploading':'failed',label:ticket.fileName?'Homebox verstuurt het bestand':'Geen bestand ontvangen',percent:ticket.fileName?65:100,phaseStartedAt:new Date().toISOString(),estimatedCompleteAt:new Date(Date.now()+(ticket.transferEstimateMs||100_000)).toISOString()},{status:ticket.fileName?'Lokale upload wordt gevolgd':'Mislukt',fileName:ticket.fileName,controllerResponse:result});if(ticket.fileName&&!ticket.localReceiver)scheduleFtpDiagnosticDownload(chargerId,ticket);if(!ticket.fileName){updateDiagnosticProgress(chargerId,ticket,{phase:'restoring',label:'Oorspronkelijke debug herstellen',percent:94});await restoreDiagnosticDebug(ticket);updateDiagnosticProgress(chargerId,ticket,{phase:'failed',label:'Geen diagnosebestand ontvangen',percent:100},{status:'Mislukt',debugRestoreStatus:ticket.debugRestoreStatus});}}catch(error){updateDiagnosticProgress(chargerId,ticket,{phase:'restoring',label:'Debug veilig herstellen',percent:94},{status:'Debuginstelling herstellen na diagnosefout'});await restoreDiagnosticDebug(ticket);updateDiagnosticProgress(chargerId,ticket,{phase:'failed',label:'Diagnose mislukt',percent:100},{status:'Mislukt',error:error.message,debugRestoreStatus:ticket.debugRestoreStatus});}},captureMs).unref();
          ...
          return send(202,{serviceResult:{status:'Uitgebreide logging gestart',steps:[ticket.freshStart?'Nieuwe controllersessie gestart':'Bestaande controllersessie gebruikt',`Volledige actuele configuratie gelezen`,`Originele chg_Debug veilig bewaard`,`Relevante modules tijdelijk op niveau 7`,`Diagnose wordt over ${durationLabel} automatisch opgevraagd`],advice:'Na ontvangst of uiterlijk na twaalf minuten wordt de oorspronkelijke debuginstelling automatisch teruggezet.'}});
        }
        const commands={
