...
export async function startEMS({port=8080,host='127.0.0.1',hardware=true,ledHardware=hardware,publicHost=null,authUser=null,authPassword=null,relayMonitorPort=8081,fleetProvider=null,fleetRouteChanger=null,fleetCommander=null}={}) {
  ...
  async function refreshHardware(){
    try{
      ...
      for(const item of fleet){if(!item.chargerConnected||!item.connectionDiagnostics?.bootAccepted){watchdogRequested.delete(item.id);continue;}if(watchdogRequested.has(item.id))continue;watchdogRequested.add(item.id);setTimeout(async()=>{const current=(typeof fleetProvider==='function'?fleetProvider():[charger]).find(row=>row.id===item.id);if(!current?.chargerConnected||current.connectedAt!==item.connectedAt||!current.connectionDiagnostics?.bootAccepted)return;const command=(action,payload)=>typeof fleetCommander==='function'?fleetCommander(item.id,action,payload):relayCommand(action,payload);let status;try{status=await command('TriggerMessage',{requestedMessage:'StatusNotification',connectorId:1});}catch{return;}if(status?.status!=='Accepted')return;try{await command('GetConfiguration',{});}catch{}try{await command('TriggerMessage',{requestedMessage:'MeterValues',connectorId:1});}catch{}},20000).unref();}
