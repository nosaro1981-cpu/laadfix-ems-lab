...
export async function startEMS({port=8080,host='127.0.0.1',hardware=true,ledHardware=hardware,publicHost=null,authUser=null,authPassword=null,relayMonitorPort=8081,fleetProvider=null,fleetRegistrar=null,fleetRouteChanger=null,fleetCommander=null,meterPollIntervalMs=30000,diagnosticCaptureMs=null,diagnosticLocalTimeoutMs=null,diagnosticReconnectWaitMs=90_000,diagnosticConfigurationTimeoutMs=10_000,diagnosticFtpUrlOverride=null,diagnosticFtpLister=null,diagnosticFtpSnapshotter=null}={}) {
  ...
  const restoreDiagnosticDebug=async ticket=>{if(!ticket?.originalDebug||ticket.debugRestored||ticket.debugRestoreBusy)return false;ticket.debugRestoreBusy=true;try{const result=await fleetCommander(ticket.chargerId,'ChangeConfiguration',{key:'chg_Debug',value:ticket.originalDebug},{timeoutMs:15_000});ticket.debugRestoreStatus=result?.status||'Onbekend';if(result?.status==='Accepted'){ticket.debugRestored=true;await clearPendingDiagnosticRestore(ticket.chargerId);return true;}return false;}catch(error){ticket.debugRestoreStatus='Mislukt: '+error.message;if(/geen antwoord|timeout|verbinding/i.test(String(error.message||''))){try{const refreshed=await fleetCommander(ticket.chargerId,'edgeReconnect',{});if(refreshed?.status==='Started')ticket.debugRestoreStatus='Stille sessie wordt automatisch vernieuwd; debugherstel volgt na heraanmelding';}catch{}}return false;}finally{ticket.debugRestoreBusy=false;}};
  ...
  async function refreshHardware(){
    try{
      ...
      for(const item of fleet){const diagnosticRunning=[...diagnosticTokens.values()].some(ticket=>ticket.chargerId===item.id&&!ticket.ending),ocppResponsive=item.connectionDiagnostics?.bootAccepted||item.connectionDiagnostics?.chargerTrafficSeen||item.commandHealth?.status==='healthy';if(item.chargerConnected&&ocppResponsive&&!item.commandHealth?.degraded&&!diagnosticRunning)void attemptPendingDiagnosticRestore(item.id);}
