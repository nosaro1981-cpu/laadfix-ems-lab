...
export async function startRelay({port=8765, monitorPort=8081, host='0.0.0.0', allowedIp='192.168.1.168', id='RBC-0000032', upstream='ws://ocpp.robo-charge.net:80/RBC-0000032',meterLogFile='meter-values.ndjson',routingFile='proxy-routing.json',pathSecret=''}={}) {
  ...
  wss.on('connection',(down,req)=>{
    ...
    for(const [ws,name]of [[up,'Robo Charge'],[down,'Homebox']]){
      ...
      ws.on('close',(code,reasonBuffer)=>{const reason=reasonBuffer?.toString()||'',time=new Date().toISOString(),unexpected=!closing&&code!==1000;if(active===session){if(ws===up)state.backendConnected=false;else state.chargerConnected=false;}state.connectionDiagnostics.lastClose={time,side:name,code,reason:reason||'Geen reden meegestuurd',clean:code===1000};if(unexpected){const failure=classifyConnectionFailure(name,reason);Object.assign(state.connectionDiagnostics,{stage:'offline',lastFailureAt:time,lastFailureType:failure.type,lastFailureLabel:failure.label,lastFailureMessage:`WebSocket ${code}${reason?' · '+reason:''}`,lastAdvice:failure.advice});state.error=`${name} sloot WebSocket ${code}${reason?' · '+reason:''}`;trace('connection_closed','error',name+' verbrak de verbinding',`WebSocket ${code}${reason?' · '+reason:''}`,time);}else trace('connection_closed',code===1000?'ok':'warning',name+' verbinding gesloten',`WebSocket ${code}${reason?' · '+reason:''}`,time);log(name+' verbinding gesloten',`code ${code}${reason?' · '+reason:''}`);close();});
    }
  });
