...
export async function startRelay({port=8765, monitorPort=8081, host='0.0.0.0', allowedIp='192.168.1.168', id='RBC-0000032', upstream='ws://ocpp.robo-charge.net:80/RBC-0000032',meterLogFile='meter-values.ndjson',routingFile='proxy-routing.json',pathSecret=''}={}) {
  ...
  function observe(raw,direction){
    ...
    try{const m=JSON.parse(raw);if(!Array.isArray(m))return;recordOcpp(m,direction,state.lastSeen);
  ...
  wss.on('connection',(down,req)=>{
    ...
    state.connectionDiagnostics.sessionId++;state.connectionDiagnostics.chargerTrafficSeen=false;state.connectionDiagnostics.backendTrafficSeen=false;state.connectionDiagnostics.stage='connecting_backend';state.connectionDiagnostics.lastBackendAttemptAt=new Date().toISOString();trace('charger_connected','ok','Homebox WebSocket geopend','OCPP 1.6-handshake geaccepteerd');trace('backend_attempt','info','Robo Charge verbinden',state.upstream);
