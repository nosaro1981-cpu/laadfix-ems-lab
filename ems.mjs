...
export const defaults = { mode: 'solar', pvW: 7500, homeW: [1000, 800, 700], phases: 3, limitA: 16, fuseA: 25, connected: true, meterOk: true, lampActive: false, lampLimitA: 6 };
export function validate(input) {
  ...
  for (const [key, min, max] of [['pvW', 0, 30000], ['limitA', 6, 32], ['fuseA', 16, 80], ['lampLimitA', 6, 32]]) {
...
export function calculate(s, previousA = 0) {
  ...
  let desiredA = Math.min(s.limitA, capacityA);
  if (s.lampActive) desiredA = Math.min(desiredA, s.lampLimitA);
  if (s.mode === 'solar') desiredA = Math.min(desiredA, Math.max(0, surplusW) / (voltage * s.phases));
  ...
  const chargeW = desiredA * voltage * s.phases;
  const gridPhaseW = baseW.map((w, i) => w + (active.includes(i) ? desiredA * voltage : 0));
...
export function createEngine() {
  ...
  function tick(now = Date.now()) {
    ...
    const chargeW = actualA * 230 * settings.phases;
    if (last !== null) energyWh += chargeW * Math.max(0, Math.min(5, (now - last) / 1000)) / 3600;
    last = now;
    const gridPhaseA = settings.homeW.map((w, i) => (w - settings.pvW / 3) / 230 + ((settings.phases === 3 || i === 0) ? actualA : 0));
