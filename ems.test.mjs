...
test('Servicediagnose onderscheidt proxy, Homebox, backoffice en storing',()=>{
 ...
 assert.match(assessService({...base,chargerConnected:false},now).summary,/laadcontroller/i);
...
test('Diagnosebestand wordt uitsluitend aan het exacte laadstation gekoppeld',()=>{
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','RBC-0000033Diag1789458432.txt'),true);
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','RBC-0000033-diag-20260915.log'),true);
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','RBC-0000099Diag1789458432.txt'),false);
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','RBC-00000330Diag1789458432.txt'),false);
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','../RBC-0000033Diag1789458432.txt'),false);
 assert.equal(diagnosticFileBelongsToStation('RBC-0000033','RBC-0000033-report.txt'),false);
});
test('Verkeerd laadstationbestand wordt geweigerd en de fout kan worden gesloten',async()=>{
 const fleet=[{id:'STATION-A',chargerConnected:true,backendConnected:true,status:'Available',activeTransaction:false}],app=await startEMS({port:0,host:'127.0.0.1',hardware:false,publicHost:'lab.example.test',authUser:'tester',authPassword:'sterk-wachtwoord',fleetProvider:()=>fleet,fleetCommander:async()=>({fileName:'STATION-BDiag123.txt'})}),base='http://127.0.0.1:'+app.port,authorization='Basic '+Buffer.from('tester:sterk-wachtwoord').toString('base64'),post=(path,body)=>fetch(base+path,{method:'POST',headers:{Authorization:authorization,Origin:base,'Content-Type':'application/json'},body:JSON.stringify(body)});
 try{
  let response=await post('/api/fleet-command',{id:'STATION-A',action:'diagnostics',enhancedDebug:false});assert.equal(response.status,400);
  let state=await(await fetch(base+'/api/state',{headers:{Authorization:authorization}})).json();assert.equal(state.diagnostics['STATION-A'].status,'Mislukt');assert.match(state.diagnostics['STATION-A'].error,/STATION-A/);
  response=await post('/api/diagnostics-dismiss',{id:'STATION-A'});assert.equal(response.status,200);
  state=await(await fetch(base+'/api/state',{headers:{Authorization:authorization}})).json();assert.equal(state.diagnostics['STATION-A'],undefined);
...
test('Snelle diagnose stopt op bestandsgrootte als een open FTP-bestand nog niet leesbaar is',()=>{
...
test('GetDiagnostics ontvangt en analyseert een controllerbestand zonder dashboardlogin',async()=>{
 ...
 try{
  ...
  const uploadPath=new URL(command.payload.location).pathname;response=await fetch(base+uploadPath,{method:'PUT',body:'MODBUS Thread active\nMeter detected: SDM630\nKWH:AD[1]RG[FC00]REC[9,9]ERR[TO]\nGSM Modem: BG95-M3\nGSM IMEI[111111111111111]\nGSM IMSI: 222222222222222\nGSM CCID[33333333333333333333]\nGSM REG:5, SQ:23,\n'});assert.equal(response.status,201);
    response=await fetch(base+'/api/state?station=DIAG-1&view=stations',{headers:{Authorization:authorization,'Accept-Encoding':'gzip'}});const state=await response.json(),report=state.diagnostics['DIAG-1'],etag=response.headers.get('etag');assert.equal(report.status,'Ontvangen');assert.equal(report.quickMode,true);assert.equal(report.progress.label,'Snelle diagnose gereed');assert.equal(report.source,'LaadFix');assert.equal(report.destination,'LaadFix beveiligde upload');assert.equal(report.minutes,5);assert.equal(report.analysis.stats.meterTimeouts,1);assert.equal(report.meterConfiguration.type,'EASTR_SDM72D');assert.equal(report.meterConfiguration.address,'1');assert.equal(report.meterAssessment.configured,'SDM72D');assert.deepEqual(report.meterAssessment.observed,['SDM630']);assert.equal(report.meterAssessment.mismatch,true);assert.equal(report.cellular.iccid,'33333333333333333333');assert.equal(report.cellular.registration,'Geregistreerd via roaming');assert.equal(state.diagnosticHistory['DIAG-1'].length,1);assert.equal(state.diagnosticHistory['DIAG-1'][0].receivedAt,report.receivedAt);assert.equal(state.diagnosticHistory['DIAG-1'][0].excerpt,undefined);assert.ok(report.excerpt.length<=4000);assert.equal(state.history.length,0);assert.ok(etag);response=await fetch(base+'/api/state?station=DIAG-1&view=stations',{headers:{Authorization:authorization,'If-None-Match':etag}});assert.equal(response.status,304);
...
test('Lokale diagnose-ontvanger gebruikt een eenmalig token en levert online analyse op',async()=>{
 ...
 try{
  ...
  response=await fetch(base+'/api/diagnostics-manual',{method:'POST',headers:{Authorization:authorization,Origin:base,'Content-Type':'application/octet-stream','X-Charger-Id':'DIAGLOCAL','X-File-Name':encodeURIComponent('DIAGLOCALDiag-handmatig.xls')},body:'KWH METER [CH][SERIAL][TYPE]:[0][9988][Eastron SDM630]\nKWH:AD[1]RG[0]R[1]OK\nftp://user:secret@example.test/file.xls'});assert.equal(response.status,201);report=(await response.json()).report;assert.equal(report.source,'Handmatige browserupload');assert.equal(report.meterIdentity.model,'SDM630');assert.equal(report.meterIdentity.serial,'9988');
  response=await fetch(base+'/api/diagnostics-text/DIAGLOCAL?receivedAt='+encodeURIComponent(report.receivedAt),{headers:{Authorization:authorization}});assert.equal(response.status,200);assert.match(response.headers.get('content-type'),/text\/plain/);assert.match(response.headers.get('content-disposition'),/DIAGLOCALDiag-handmatig\.txt/);const text=await response.text();assert.match(text,/Eastron SDM630/);assert.doesNotMatch(text,/secret|user/);assert.match(text,/FTP-adres/);
 }finally{await app.close();}
});
...
test('Werkelijke herstart wordt gevolgd wanneer de Homebox soft reset als Rejected meldt',async()=>{
 ...
 const app=await startEMS({port:0,host:'127.0.0.1',hardware:false,publicHost:'lab.example.test',authUser:'tester',authPassword:'sterk-wachtwoord',diagnosticCaptureMs:1,fleetProvider:()=>fleet,fleetCommander:async(id,action,payload)=>{calls.push({action,payload});if(action==='Reset'){fleet[0].connectionDiagnostics.sessionId=2;return{status:'Rejected'};}if(action==='GetConfiguration')return{configurationKey:[{key:'chg_Debug',value:DIAGNOSTIC_DEBUG_BASE},{key:'chg_KWH1',value:'EASTR_SDM630,1,9600,N,1'}]};if(action==='GetDiagnostics')return{fileName:'RESETFALLBACKDiag.xls'};return{status:'Accepted'};}}),base='http://127.0.0.1:'+app.port,authorization='Basic '+Buffer.from('tester:sterk-wachtwoord').toString('base64');
