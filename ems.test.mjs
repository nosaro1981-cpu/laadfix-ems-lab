...
test('Geweigerde soft reset laat een lokale diagnose met een exact nieuw tijdvak doorgaan',async()=>{
 ...
 const app=await startEMS({port:0,host:'127.0.0.1',hardware:false,publicHost:'lab.example.test',authUser:'tester',authPassword:'sterk-wachtwoord',diagnosticCaptureMs:1,fleetProvider:()=>fleet,fleetCommander:async(id,action,payload)=>{calls.push({action,payload});if(action==='Reset')return{status:'Rejected'};if(action==='GetConfiguration')return{configurationKey:[{key:'chg_Debug',value:DIAGNOSTIC_DEBUG_BASE},{key:'chg_KWH1',value:'EASTR_SDM630,1,9600,N,1'}]};if(action==='GetDiagnostics')return{fileName:'RESETFALLBACK.xls'};return{status:'Accepted'};}}),base='http://127.0.0.1:'+app.port,authorization='Basic '+Buffer.from('tester:sterk-wachtwoord').toString('base64');
 try{const response=await fetch(base+'/api/fleet-command',{method:'POST',headers:{Authorization:authorization,Origin:base,'Content-Type':'application/json'},body:JSON.stringify({id:'RESETFALLBACK',action:'diagnostics',localReceiverIp:'192.168.1.20',durationSeconds:30,freshStart:true,debugModules:['modbus']})});assert.equal(response.status,202);const body=await response.json();assert.match(body.serviceResult.steps[0],/Soft reset Rejected/);for(let i=0;i<30&&!calls.some(row=>row.action==='GetDiagnostics');i++)await new Promise(resolve=>setTimeout(resolve,10));assert.ok(calls.some(row=>row.action==='GetDiagnostics'));const state=await(await fetch(base+'/api/state',{headers:{Authorization:authorization}})).json();assert.equal(state.diagnostics.RESETFALLBACK.freshStart,false);assert.match(state.diagnostics.RESETFALLBACK.sessionNote,/exact nieuw tijdvak/);}
 finally{await app.close();}
});
