...
export async function startCloud({port=Number(process.env.PORT||process.env.APP_PORT||process.env.NODE_PORT||(process.env.RENDER?10000:8080)),host=process.env.RENDER?'0.0.0.0':'127.0.0.1',publicHost=process.env.PUBLIC_HOST||process.env.RENDER_EXTERNAL_HOSTNAME,id=process.env.OCPP_ID||'RBC-0000032',pathSecret=process.env.OCPP_PATH_SECRET,upstream=process.env.OCPP_UPSTREAM||`ws://ocpp.robo-charge.net:80/${id}`,upstreamTemplate=process.env.OCPP_UPSTREAM_TEMPLATE||'ws://ocpp.robo-charge.net:80/#OSN#',authUser=process.env.DASHBOARD_USER,authPassword=process.env.DASHBOARD_PASSWORD,meterLogFile=process.env.METER_LOG_FILE||'data/meter-values.ndjson',routingFile=process.env.ROUTING_FILE||'data/proxy-routing.json'}={}){
  ...
  const fleetState=()=>Array.from(relays.entries()).flatMap(([chargerId,value])=>value?.state?[{id:chargerId,chargerConnected:value.state.chargerConnected,backendConnected:value.state.backendConnected,status:value.state.connectors?.[1]?.status||'Onbekend',errorCode:value.state.connectors?.[1]?.errorCode||null,lastSeen:value.state.lastSeen,lastHeartbeat:value.state.lastHeartbeat,lastStatusNotification:value.state.lastStatusNotification,lastMeterValues:value.state.lastMeterValues,lastMeterForwarded:value.state.lastMeterForwarded,meterHistoryCount:value.state.meterHistoryCount,forwarded:value.state.forwarded,received:value.state.received,upstream:value.state.upstream,error:value.state.error,boot:value.state.boot,connectors:value.state.connectors,meterValues:value.state.meterValues,meterHistory:value.state.meterHistory,events:value.state.events?.slice(0,30),activeTransaction:value.state.activeTransaction,transactionId:value.state.transactionId,lastLocalCommand:value.state.lastLocalCommand,configuration:value.state.configuration,configurationUpdatedAt:value.state.configurationUpdatedAt,diagnosticsStatus:value.state.diagnosticsStatus,diagnosticsStatusAt:value.state.diagnosticsStatusAt,remoteDiagnostics:value.state.remoteDiagnostics,connectionStats:value.state.connectionStats,connectionDiagnostics:value.state.connectionDiagnostics}]:[]);
