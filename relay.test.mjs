...
test('Homebox en backoffice ontvangen exact dezelfde berichten via relay', {timeout:10000},async()=>{
 ...
 const trigger=JSON.parse((await triggered)[0].toString());assert.equal(trigger[2],'GetConfiguration');assert.deepEqual(trigger[3],{key:['HeartbeatInterval']});charger.send(JSON.stringify([3,trigger[1],{configurationKey:[{key:'HeartbeatInterval',readonly:false,value:'900'}]}]));
...
test('Een nieuwe Render-relay controleert de bestaande Homeboxsessie actief', {timeout:5000},async()=>{
 ...
 try{
  ...
  const call=JSON.parse((await once(charger,'message'))[0].toString());assert.equal(call[2],'GetConfiguration');assert.deepEqual(call[3],{key:['HeartbeatInterval']});
  charger.send(JSON.stringify([3,call[1],{configurationKey:[{key:'HeartbeatInterval',readonly:false,value:'900'}]}]));await new Promise(resolve=>setTimeout(resolve,10));
...
test('Een onbeantwoorde backofficeopdracht laat beide sockets open', {timeout:5000},async()=>{
 const backend=new WebSocketServer({port:0,host:'127.0.0.1',handleProtocols:()=> 'ocpp1.6'});await once(backend,'listening');
 ...
 try{
  ...
  up.send('[2,"probe-a","TriggerMessage",{"requestedMessage":"Heartbeat"}]');await once(charger,'message');await new Promise(resolve=>setTimeout(resolve,45));
  assert.equal(app.state.chargerConnected,true);assert.equal(app.state.backendConnected,true);assert.equal(charger.readyState,WebSocket.OPEN);assert.equal(up.readyState,WebSocket.OPEN);assert.equal(app.state.commandHealth.degraded,true);assert.equal(app.state.commandHealth.lastTimeoutAction,'TriggerMessage');assert.ok(!app.state.connectionTimeline.some(row=>row.type==='backend_auto_reconnect'));
 }finally{charger?.terminate();up?.terminate();await app.close();await new Promise(r=>backend.close(r));}
});
